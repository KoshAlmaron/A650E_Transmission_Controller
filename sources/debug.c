...
static void print_config_gear3_delay();
...
static void print_config_gear3_delay() {
	...
	for (uint8_t i = 0; i < COLUMN_COUNT; i++) {
		...
		lcd_set_cursor(3, i * 4);
		Value = MIN(999, SLUGear3DelayGraph[StartCol + i]);
		snprintf(LCDArray, 4, "%3u", Value);
		lcd_send_string(LCDArray, 3);

		if (CursorPos == StartCol + i) {
			uint8_t Step = 50;
			if (!CursorPos) {Step = 10;}
			Value = SLUGear3DelayGraph[CursorPos];
			if (ValueDelta < 0) {
				if (Value > Step) {Value += ValueDelta * Step;}
				else {Value = 0;}
			}
			if (ValueDelta > 0) {Value = MIN(900, Value + ValueDelta * Step);}
